...
class Arquivo:
    ...
    def write(self, dados, replicas, password=None):
        if password == "god" or reduce(
                lambda autorizacao, lock: autorizacao or (lock.password == password and lock.tipo == "w"),
                self.locks, False):
            ...
            for line in hosts:
                self.replicas.append(
                    xmlrpc.client.ServerProxy(
                        "http://%s:8000/" % line.strip()))

    def open(self, tipo, name):
